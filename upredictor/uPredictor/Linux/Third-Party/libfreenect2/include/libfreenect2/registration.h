...
void apply(const unsigned int *rgb_data, const float *depth_data
	...
	, const bool enable_filter = true, float *filter_map = NULL
   ...
   */
	void getPointXYZ(
		const Frame* undistorted, int r, int c, float& x, float& y, float& z)
		const;

  /** Construct a 3-D point cloud.
   * @param undistorted Undistorted depth frame from apply().
   * @param pts PointXYZ array to be filled in
   * @param rows Rows in depth image.
   * @param cols Columns in depth image.
	*/
