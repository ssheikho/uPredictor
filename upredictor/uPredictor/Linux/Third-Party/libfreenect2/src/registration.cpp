...
public:
	RegistrationImpl(Freenect2Device::IrCameraParams depth_p
		, Freenect2Device::ColorCameraParams rgb_p);

	void apply(int dx, int dy, float dz, float& cx, float &cy) const;
	//void apply(const Frame* rgb, const Frame* depth, Frame* undistorted
	//	, Frame* registered, const bool enable_filter, Frame* bigdepth
	//	, int* color_depth_map) const;

	void apply(const unsigned int *rgb_data, const float *depth_data
		, float *undistorted_data, unsigned int *registered_data
		, int *color_depth_map = NULL) const;

	void undistortDepth(const Frame *depth, Frame *undistorted) const;
	void getPointXYZRGB (const Frame* undistorted, const Frame* registered
		, int r, int c, float& x, float& y, float& z, float& rgb) const;
	void getPointXYZ (const Frame* undistorted, int r, int c
		, float& x, float& y, float& z) const;
	void getPointsXYZ (const float* undistorted_data, PointXYZ *pts
...
void Registration::apply(const Frame *rgb, const Frame *depth
	, Frame *undistorted, Frame *registered, const bool enable_filter
	, Frame *bigdepth, int *color_depth_map) const
{
	if(bigdepth) {
		apply(	(unsigned int *)rgb->data
			, (const float *)depth->data, (float *)undistorted->data
			, (unsigned int *)registered->data
			, enable_filter, (float *)bigdepth->data, color_depth_map);
	} else {
		float *bdd = new float[1920*1082];
		apply(	(unsigned int *)rgb->data
			, (const float *)depth->data, (float *)undistorted->data
			, (unsigned int *)registered->data, enable_filter, bdd, color_depth_map);
		delete[] bdd;
	}

}

void Registration::apply(const unsigned int *rgb_data, const float *depth_data
	, float *undistorted_data, unsigned int *registered_data
	, const bool enable_filter, float *filter_map, int *color_depth_map)
	const {
	impl_->apply(rgb_data, depth_data, undistorted_data, registered_data
		, color_depth_map);
}

void RegistrationImpl::apply(
	const unsigned int *rgb_data, const float *depth_data
	, float *undistorted_data, unsigned int *registered_data
	, int *color_depth_map) const {
	//std::cout << distort_map << std::endl;
...
void RegistrationImpl::getPointXYZ (const Frame *undistorted, int r, int c
	, float &x, float &y, float &z) const {
...
{
	//std::cout << "A:	" << distort_map << std::endl;
  float mx, my;
  int ix, iy, index;
  float rx, ry;
