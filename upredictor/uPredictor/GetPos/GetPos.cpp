...
void GetPos::parsedata(const string& filename) {
	...
	while (getline(f, s))  // get the rows of the csv file
	{
		...
		istringstream iss(s);
		while (getline(iss, s, ','))	// get each cell in the row
		{
			string fieldvalue = "";
			istringstream(s) >> fieldvalue;		// asign element to fieldvalue

			if (fieldvalue == "" && colNum < 3) {  // skip first two cells of each row
				markers[colNum] = fieldvalue;
				colNum++;
				continue;
			}
			else if (i == 2) { // third row have the marker's names
				markers[colNum] = fieldvalue;
				...
				if (markers[currColNum] == "") {	// skip values until next marker
					...
					if (fieldvalue == "") {
						...
						currX = fieldvalue; // same column as the marker name
						getline(iss, s, ',');
...
const GetPos::Trajectory GetPos::pos(const string& filename, const int& Frame) {
...
const vector<double> GetPos::get(const string& MarkerName, const int& Frame) {
	...
	for(Marker currMarker: GetPos::MarkerList)
	{
...
int main()
{
	...
	vector <double> testoutput = data.get("Jorge:RElb", 100);
