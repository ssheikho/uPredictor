...
template void bilateralFilter<unsigned short>(const unsigned short * depthIn, float * depthOut, const int width, const int height, const float sigmaDomain, const float sigmaRange);
