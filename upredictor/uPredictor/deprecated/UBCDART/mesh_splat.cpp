...
void splatSolidMesh(const dart::Mesh & mesh, dart::Grid3D<float> & sdf) {
    ...
    for (int f = 0; f<mesh.nFaces; f++) {
        ...
        for (int z = max(0.0,floor((min_z-sdf.offset.z)/sdf.resolution)); z < min((double)sdf.dim.z,ceil((max_z-sdf.offset.z)/sdf.resolution)); z++) {
            for (int y = max(0.0,floor((min_y-sdf.offset.y)/sdf.resolution)); y < min((double)sdf.dim.y,ceil((max_y-sdf.offset.y)/sdf.resolution)); y++) {
                for (int x = max(0.0,floor((min_x-sdf.offset.x)/sdf.resolution)); x < min((double)sdf.dim.x,ceil((max_x-sdf.offset.x)/sdf.resolution)); x++) {
