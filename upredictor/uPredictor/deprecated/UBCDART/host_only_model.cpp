...
HostOnlyModel::HostOnlyModel(ModelRenderer *renderer) : Model(renderer, 6) {
    _nFrames = 1;
...
float HostOnlyModel::evaluateExpression(const std::string & expression) {
    ...
    double *varValues = new double [varCount];
...
int HostOnlyModel::addFrame(int parent, JointType type,
	...
	float jointMinF = std::stof(jointMin);
	float jointMaxF = std::stof(jointMax);

    _jointLimits.push_back(
		make_float2(jointMinF, jointMaxF
			//evaluateExpression(jointMin),
	...
	float posXF = std::stof(posX);
	float posYF = std::stof(posY);
	float posZF = std::stof(posZ);

    float3 pos = make_float3(posXF, posYF, posZF
		//evaluateExpression(posX),
		//evaluateExpression(posY),
		//evaluateExpression(posZ)
		);
    _positions.push_back(pos);

	float orXF = std::stof(orX);
	float orYF = std::stof(orY);
	float orZF = std::stof(orZ);

    float3 orientation = make_float3(orXF, orYF, orZF
		//evaluateExpression(orX),
  //      evaluateExpression(orY),
  //      evaluateExpression(orZ)
		);
    _orientations.push_back(orientation);

	float axisXF = std::stof(axisX);
	float axisYF = std::stof(axisY);
	float axisZF = std::stof(axisZ);

    float3 axis = make_float3(axisXF, axisYF, axisZF
		//evaluateExpression(axisX),
...
void HostOnlyModel::addGeometry(
	int frame
	, GeomType geometryType
	, std::string sx, std::string sy, std::string sz
	...
	float sxf = std::stof(sx);// evaluateExpression(sx);
	float syf = std::stof(sy);
	float szf = std::stof(sz);
	float txf = std::stof(tx);
	float tyf = std::stof(ty);
	float tzf = std::stof(tz);
	float rxf = std::stof(rx);
	float ryf = std::stof(ry);
	float rzf = std::stof(rz);

    addGeometry(
		frame, geometryType
		, sxf, syf, szf, txf, tyf, tzf, rxf, ryf, rzf,
        red, green, blue, meshFilename);
...
void HostOnlyModel::computeStructure() {
    ...
    for (int f=0; f<getNumFrames()-1; ++f) {
		...
		_positions[f].x = std::stof(_frameParamExpressions[f][0]);
		_positions[f].y = std::stof(_frameParamExpressions[f][1]);
		_positions[f].z = std::stof(_frameParamExpressions[f][2]);
		_orientations[f].x = std::stof(_frameParamExpressions[f][3]);
		_orientations[f].y = std::stof(_frameParamExpressions[f][4]);
		_orientations[f].z = std::stof(_frameParamExpressions[f][5]);
		_axes[f].x = std::stof(_frameParamExpressions[f][6]);
		_axes[f].y = std::stof(_frameParamExpressions[f][7]);
		_axes[f].z = std::stof(_frameParamExpressions[f][8]);
		_jointLimits[f] =
			make_float2(
				std::stof(_frameParamExpressions[f][9]),
				std::stof(_frameParamExpressions[f][10]));
    }
    ...
    for (int g=0; g<getNumGeoms(); ++g) {
		...
		float3 scale = make_float3(std::stof(_geomParamExpressions[g][0]),
			std::stof(_geomParamExpressions[g][1]),
			std::stof(_geomParamExpressions[g][2]));
		_geomScales[g] = scale;

		if (_modelVersion == 0) {
			_geomTransforms[g] = SE3Fromse3(se3(std::stof(_geomParamExpressions[g][3]),
				std::stof(_geomParamExpressions[g][4]),
				std::stof(_geomParamExpressions[g][5]),
				std::stof(_geomParamExpressions[g][6]),
				std::stof(_geomParamExpressions[g][7]),
				std::stof(_geomParamExpressions[g][8])));
		}
		else {
			_geomTransforms[g] = SE3FromTranslation(std::stof(_geomParamExpressions[g][3]),
				std::stof(_geomParamExpressions[g][4]),
				std::stof(_geomParamExpressions[g][5]))*
				SE3FromEuler(make_float3(std::stof(_geomParamExpressions[g][8]),
					std::stof(_geomParamExpressions[g][7]),
					std::stof(_geomParamExpressions[g][6])));
		}
...
void HostOnlyModel::voxelizeFrame(Grid3D<float> &sdf, const int frame, const float fg, const float bg, const float resolution, const float padding, const float Rc) {
    ...
    for (int g = 0; g < getFrameNumGeoms(frame); g++) {
        ...
        case MeshType:
		{
			int meshNumber = getMeshNumber(geom);
			const Mesh& mesh = _renderer->getMesh(meshNumber);
			Mesh transformedMesh(mesh);
