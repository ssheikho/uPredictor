...
namespace dart {
	...
	//ModelRenderer * UBCDARTModel::_renderer;

	UBCDARTModel::UBCDARTModel(UBCDARTModelRenderer *renderer
		, const UBCDARTModel & copy) : _renderer(renderer){
	...
	void UBCDARTModel::render(const char alpha) const {

		renderGeometries(&UBCDARTModel::renderColoredPrerender, (const char*)&alpha);

	}

	void UBCDARTModel::renderColoredPrerender(const int frameNumber, const int geomNumber, const char * args) const {
	...
	void UBCDARTModel::renderWireframe() const {

		renderGeometries(&UBCDARTModel::renderWireframePrerender, 0, &UBCDARTModel::renderWireFramePostrender);

	}

	void UBCDARTModel::renderSkeleton(const float sphereSize, const float lineSize) const {
	...
	void UBCDARTModel::renderWireframePrerender(const int frameNumber, const int geomNumber, const char * args) const {

		glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);

	}

	void UBCDARTModel::renderWireFramePostrender() const {

		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);

	}

	void UBCDARTModel::renderLabeled(const int modelNumber) const {

		renderGeometries(&UBCDARTModel::renderLabeledPrerender, (char *)&modelNumber);

	}


	void UBCDARTModel::renderLabeledPrerender(const int frameNumber, const int geomNumber, const char* args) const {
	...
	void UBCDARTModel::renderGeometries(void (UBCDARTModel::*prerenderFunc)(const int, const int, const char *) const,
		const char* args,
		void (UBCDARTModel::*postrenderFunc)() const) const {
	...
	void UBCDARTModel::renderVoxels(const float levelSet) const {

		renderFrames(&UBCDARTModel::renderVoxelizedFrame, (char*)&levelSet);

	}

	void UBCDARTModel::renderVoxelizedFrame(const int frameNumber, const char* args) const {
	...
	void UBCDARTModel::renderSdf(const dart::Grid3D<float> & sdf, float levelSet) const {
	...
	void UBCDARTModel::renderCoordinateFrames(const float size) const {
		...
		renderFrames(&UBCDARTModel::renderCoordinateFrame, (char*)&size);
		//    glEnable(GL_DEPTH_TEST);

	}

	void UBCDARTModel::renderCoordinateFrame(const int frameNumber, const char * args) const {
	...
	void UBCDARTModel::renderFrames(void (UBCDARTModel::*renderFrameFunc)(const int, const char *) const, const char * args) const {
	...
	void UBCDARTModel::renderFrameParents() const {
	...
	void UBCDARTModel::getModelJacobianOfModelPoint(const float4 & modelPoint, const int frame, std::vector<float3> & J3D) const {
	...
	void UBCDARTModel::getArticulatedBoundingBox(float3 & mins, float3 & maxs, const float modelSdfPadding, const int nSweepPoints) {
		...
		for (int s = 1; s<getNumSdfs(); ++s) {
			...
			float *pose = new float[getPoseDimensionality()];
			for (int c = 0; c<jointAngleCombinations.size(); ++c) {
