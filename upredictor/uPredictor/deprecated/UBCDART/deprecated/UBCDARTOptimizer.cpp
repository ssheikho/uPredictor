...
#include "UBCDARTModToObs.h"
#include "UBCDARTObsToMod.h"
...
namespace dart {
	...
	UBCDARTOptimizer::UBCDARTOptimizer(const DepthSourceBase * depthSource
	...
	UBCDARTOptimizer::UBCDARTOptimizer(const int depthWidth, const int depthHeight, const float2 focalLength,
		const int predictionWidth, const int predictionHeight) {
	...
	void UBCDARTOptimizer::init(const int depthWidth, const int depthHeight, const float2 focalLength,
		const int predictionWidth, const int predictionHeight) {
		...
		_predictionRenderer = new UBCDARTPredictionRenderer(predictionWidth, predictionHeight, focalLength);
	...
	UBCDARTOptimizer::~UBCDARTOptimizer() {
	...
	void UBCDARTOptimizer::unpack(Eigen::VectorXf & eJ,
		Eigen::MatrixXf & JTJ,
		float & e,
		float * sys,
	...
	void UBCDARTOptimizer::generateObsSdf(UBCDARTMirroredModel & model,
		const UBCDARTObservation & observation,
	...
	void UBCDARTOptimizer::generateObsSdfSplatZeroAndDistanceTransform(UBCDARTMirroredModel & model,
		const UBCDARTObservation & observation,
		const OptimizationOptions & opts) {
	...
	void UBCDARTOptimizer::generateObsSdfDirectTruncated(UBCDARTMirroredModel & model,
		const UBCDARTObservation & observation,
		const OptimizationOptions & opts) {
	...
	void UBCDARTOptimizer::computeObsToModContribution(
		Eigen::VectorXf & eJ, Eigen::MatrixXf & JTJ, float & error,
		const UBCDARTMirroredModel & model, const Pose & pose,
		const OptimizationOptions & opts, const UBCDARTObservation & observation) {
	...
	void UBCDARTOptimizer::computeModToObsContribution(
		Eigen::VectorXf & eJ, Eigen::MatrixXf & JTJ, float & error
		, const UBCDARTMirroredModel & model, const Pose & pose,
		const SE3 & T_obsSdf_c,	const OptimizationOptions & opts) {
	...
	void UBCDARTOptimizer::computeSelfIntersectionContribution(
		Eigen::VectorXf & eJ, Eigen::MatrixXf & JTJ, float & error,
		const UBCDARTMirroredModel & model, const Pose & pose,
		const OptimizationOptions & opts,
	...
	void UBCDARTOptimizer::computeIntersectionContribution(Eigen::VectorXf & eJ, Eigen::MatrixXf & JTJ, float & error,
		const UBCDARTMirroredModel & srcModel, const UBCDARTMirroredModel & dstModel,
	...
	void UBCDARTOptimizer::optimizePose(UBCDARTMirroredModel & model,
		Pose & pose,
		...
		UBCDARTObservation observation(dObsVertMap, dObsNormMap, width, height);
	...
	void UBCDARTOptimizer::optimizePoses(std::vector<UBCDARTMirroredModel *> & models,
		...
		int sysSize = 0;
		int *modelOffsets = new int[nModels];
		int *priorOffsets = new int[nPriors];
		for (int m = 0; m<nModels; ++m) {
		...
		for (int iteration = 0; iteration < opts.numIterations; ++iteration) {
			...
			for (int m = 0; m<nModels; ++m) {
				...
				UBCDARTMirroredModel & model = *models[m];
				Pose & pose = poses[m];
