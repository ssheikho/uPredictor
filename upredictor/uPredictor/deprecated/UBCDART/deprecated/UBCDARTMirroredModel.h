#ifndef UBC_DART_MIRRORED_MODEL_H
#define UBC_DART_MIRRORED_MODEL_H
...
namespace dart {
	...
	class  UBCDARTMirroredModel : public UBCDARTModel {
	public:
		UBCDARTMirroredModel(const UBCDARTHostOnlyModel & copy,
			const uint3 obsSdfDim,
			const float obsSdfRes,
			const float3 obsSdfOffset = make_float3(0, 0, 0),
			const int modelID = -1);
		~UBCDARTMirroredModel();
