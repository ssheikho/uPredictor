...
namespace dart {
	...
	UBCDARTTracker::UBCDARTTracker(UBCDARTModelRenderer *renderer) : 
		_renderer(renderer)
		, _depthSource(0), _pcSource(0)
		, _optimizer(0)
		,	_T_mcs(0), _T_fms(0)
		, _sdfFrames(0), _sdfs(0), _nSdfs(0)
		, _distanceThresholds(0), _normalThresholds(0)
		, _planeOffsets(0), _planeNormals(0), _dampingMatrices(0) {}

	UBCDARTTracker::~UBCDARTTracker() {
	...
	bool UBCDARTTracker::addModel(const std::string & filename,
		...
		UBCDARTHostOnlyModel model(_renderer);
		...
		_mirroredModels.push_back(new UBCDARTMirroredModel(model,
		...
		UBCDARTMirroredModel & mm = *_mirroredModels.back();
	...
	bool UBCDARTTracker::addDepthSource(DepthSource<DepthType, ColorType> * depthSource) {
	...
	template bool UBCDARTTracker::addDepthSource<float, uchar3>(DepthSource<float, uchar3> * depthSource);
	template bool UBCDARTTracker::addDepthSource<ushort, uchar3>(DepthSource<ushort, uchar3> * depthSource);

	void UBCDARTTracker::updateModel(const int modelNum,
		const float modelSdfResolution,
		const float modelSdfPadding,
		const int obsSdfSize,
		...
		UBCDARTMirroredModel * newModel = new UBCDARTMirroredModel(model,
			make_uint3(obsSdfSize),
			obsSdfResolution,
			obsSdfCenter,
			modelID);
	...
	void UBCDARTTracker::stepForward() {
	...
	void UBCDARTTracker::stepBackward() {
	...
	void UBCDARTTracker::setFrame(const int frame) {
	...
	void UBCDARTTracker::subtractPlane(const float3 planeNormal,
	...
	void UBCDARTTracker::optimizePose(const int modelNum) {
	...
	void UBCDARTTracker::optimizePoses() {
	...
	void UBCDARTTracker::setIntersectionPotentialMatrix(const int modelNum, const int * mx) {
