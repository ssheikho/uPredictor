#ifndef UBC_DART_OPTIMIZER_H
#define UBC_DART_OPTIMIZER_H
...
namespace dart {
	...
	struct UBCDARTObservation {
		const float4 * dVertMap, *dNormMap;
		const int width, height;
		UBCDARTObservation(
			const float4 * dVertMap_, const float4 * dNormMap_
			, const int width_, const int height_) :
			dVertMap(dVertMap_), dNormMap(dNormMap_)
			, width(width_), height(height_) { }
	};

	class UBCDARTOptimizer {
	public:
		UBCDARTOptimizer(const DepthSourceBase * depthSource,
			const int predictionWidth = -1, const int predictionHeight = -1);
		UBCDARTOptimizer(const int depthWidth, const int depthHeight, const float2 focalLength,
			const int predictionWidth = -1, const int predictionHeight = -1);
		~UBCDARTOptimizer();

		void optimizePose(UBCDARTMirroredModel & model,
			Pose & pose,
		...
		void optimizePoses(std::vector<UBCDARTMirroredModel *> & models,
		...
		*/
		...
		void computePredictedPointCloud(const std::vector<UBCDARTMirroredModel*> & models) {

			std::vector<const UBCDARTMirroredModel *> constModelPtrs(models.size());
		...
		void debugPredictionRay(const std::vector<UBCDARTMirroredModel*> & models, const int x, const int y,
	...
	private:
		...
		void generateObsSdf(UBCDARTMirroredModel & model,
			const UBCDARTObservation & observation,
			const OptimizationOptions & opts);

		void generateObsSdfSplatZeroAndDistanceTransform(UBCDARTMirroredModel & model,
			const UBCDARTObservation & observation,
			const OptimizationOptions & opts);

		void generateObsSdfDirectTruncated(UBCDARTMirroredModel & model,
			const UBCDARTObservation & observation,
			const OptimizationOptions & opts);
		...
		void computeModToObsContribution(Eigen::VectorXf & eJ, Eigen::MatrixXf & JTJ, float & error,
			const UBCDARTMirroredModel & model, const Pose & pose, const SE3 & T_obsSdf_c,
			const OptimizationOptions & opts);
		...
		UBCDARTPredictionRenderer * _predictionRenderer;
