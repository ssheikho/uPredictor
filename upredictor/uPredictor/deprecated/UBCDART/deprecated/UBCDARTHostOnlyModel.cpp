...
namespace dart {
	...
	UBCDARTHostOnlyModel::UBCDARTHostOnlyModel(
		UBCDARTModelRenderer *renderer) :
		UBCDARTModel(renderer, 6) {
		_nFrames = 1;
	...
	float UBCDARTHostOnlyModel::evaluateExpression(const std::string & expression) {
	...
	int UBCDARTHostOnlyModel::addFrame(int parent, JointType type,
	...
	void UBCDARTHostOnlyModel::addGeometry(int frame, GeomType geometryType, std::string sx, std::string sy, std::string sz, std::string tx, std::string ty, std::string tz, std::string rx, std::string ry, std::string rz, unsigned char red, unsigned char green, unsigned char blue, const std::string meshFilename) {
	...
	void UBCDARTHostOnlyModel::addGeometry(int frame, GeomType geometryType, float sx, float sy, float sz, float tx, float ty, float tz, float rx, float ry, float rz, unsigned char red, unsigned char green, unsigned char blue, const std::string meshFilename) {
	...
	void UBCDARTHostOnlyModel::computeStructure() {
	...
	// TODO
	void UBCDARTHostOnlyModel::voxelize(float resolution, float padding, std::string cacheFile) {
	...
	void UBCDARTHostOnlyModel::voxelize2(float resolution, float padding, std::string cacheFile) {
	...
	void UBCDARTHostOnlyModel::setGeometryColor(int geomNumber, unsigned char color[]) {
		_geomColors[geomNumber] = make_uchar3(color[0], color[1], color[2]);
	}

	void UBCDARTHostOnlyModel::setGeometryColor(int geomNumber, unsigned char red, unsigned char green, unsigned char blue) {
		_geomColors[geomNumber] = make_uchar3(red, green, blue);
	}


	void UBCDARTHostOnlyModel::setJointLimits(const int joint, const float min, const float max) {
	...
	void UBCDARTHostOnlyModel::setArticulation(const float * pose) {
	...
	void UBCDARTHostOnlyModel::setPose(const Pose & pose) {
	...
	void UBCDARTHostOnlyModel::setGeometryScale(const int geom, const float3 scale) {
		_geomScales[geom] = scale;
	}

	void UBCDARTHostOnlyModel::setGeometryTransform(const int geom, const SE3 &T) {
		_geomTransforms[geom] = T;
	}

	void UBCDARTHostOnlyModel::setLinkParent(const int link, const int parent) {
		_parents[link] = parent;
	}

	void UBCDARTHostOnlyModel::setVoxelGrid(Grid3D<float> &grid, const int link) {
	...
	void UBCDARTHostOnlyModel::voxelizeFrame(Grid3D<float> &sdf, const int frame, const float fg, const float bg, const float resolution, const float padding, const float Rc) {
