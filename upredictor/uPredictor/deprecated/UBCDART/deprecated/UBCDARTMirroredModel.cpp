...
namespace dart {
	...
	UBCDARTMirroredModel::UBCDARTMirroredModel(const UBCDARTHostOnlyModel & copy,
		const uint3 obsSdfDim,
		const float obsSdfRes,
		const float3 obsSdfOffset,
		const int modelID) : UBCDARTModel(copy) {
	...
	UBCDARTMirroredModel::~UBCDARTMirroredModel() {
	...
	void UBCDARTMirroredModel::setArticulation(const float * pose) {
	...
	void UBCDARTMirroredModel::setPose(const Pose & pose) {
