...
//    }
    ...
    for (int i=0; i<NumPrimitives; ++i) {
        ...
        glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB,_primitiveVBOs[i]);
        glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB,primitive->nVertices*sizeof(float3),primitive->vertices,GL_STATIC_DRAW_ARB);

        glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB,_primitiveNBOs[i]);
        glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB,primitive->nVertices*sizeof(float3),primitive->normals,GL_STATIC_DRAW_ARB);
...
void ModelRenderer::renderMesh(const unsigned int meshNum) const {
...
const Mesh & ModelRenderer::getMesh(const unsigned int meshNum) const {
