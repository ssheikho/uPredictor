...
const inline std::string getVersionString() {
	return stringFormat("%d.%d.%d",versionMajor,versionMinor,versionRevision);
}
...
class UBCDARTTracker :
	...
	, public TypeBroadcaster<UBCDARTTracker *> {

public:
    UBCDARTTracker(KIPointCloudSource &pcSource, CameraModel &cm);
    ~UBCDARTTracker();
    ...
    inline int getNumModels() const {
		return _mirroredModels.size();
	}

	inline const MirroredModel & getModel(const int modelNum) const {
		return * _mirroredModels[modelNum];
	}

    inline MirroredModel & getModel(const int modelNum) {
		return * _mirroredModels[modelNum];
	}

    inline const Pose & getPose(const int modelNum) const {
		return _estimatedPoses[modelNum];
	}

    inline Pose & getPose(const int modelNum) {
		return _estimatedPoses[modelNum];
	}
    ...
    inline const float4 * getHostVertMap() {
		return _pcSource.getHostVertMap();
	}

    inline const float4 * getHostNormMap() {
		return _pcSource.getHostNormMap();
	}
    ...
    inline const void setFilteredNorms(const bool filteredNorms) {
		_pcSource.setFilteredNorms(filteredNorms);
	}

    inline const void setFilteredVerts(const bool filteredVerts) {
		_pcSource.setFilteredVerts(filteredVerts);
	}

    inline const void setSigmaDepth(const float sigmaDepth) {
		_pcSource.setSigmaDepth(sigmaDepth);
	}

    inline const void setSigmaPixels(const float sigmaPixels) {
		_pcSource.setSigmaPixels(sigmaPixels);
	}
    ...
    void cropBox(const float3 & min, const float3 & max) {
		_pcSource.cropBox(min,max);
	}
	
    void maskPointCloud(const int * deviceMask) {
		_pcSource.maskPointCloud(deviceMask);
	}
