...
UBCDARTTracker::UBCDARTTracker(KIPointCloudSource &pcSource, CameraModel &cm) :
	_pcSource(pcSource), _optimizer(new UBCDARTOptimizer(cm))
	, _T_mcs(0), _T_fms(0), _sdfFrames(0), _sdfs(0), _nSdfs(0)
	, _distanceThresholds(0), _normalThresholds(0), _planeOffsets(0)
	, _planeNormals(0), _dampingMatrices(0) {}
...
void UBCDARTTracker::processType(SimplePool<KinectFrame>::PooledPtr t) {
	
	_pcSource.processType(t);
	optimizePose(0);
	//for(int i = 0; i < _mirroredModels.size(); i++)	updatePose(i);
	fireUpdate(this);
}

void UBCDARTTracker::onStartup() {}

bool UBCDARTTracker::addModel(
...
void UBCDARTTracker::updateModel(
	const int modelNum
	, const float modelSdfResolution
	, const float modelSdfPadding
	, const int obsSdfSize
	...
	_T_fms->hostPtr()[modelNum] = newModel->getDeviceTransformsModelToFrame();
	_T_fms->syncHostToDevice();

	_sdfFrames->hostPtr()[modelNum] = newModel->getDeviceSdfFrames();          
	_sdfFrames->syncHostToDevice();

	_sdfs->hostPtr()[modelNum] = newModel->getDeviceSdfs();                    
...
void UBCDARTTracker::subtractPlane(
	...
	subtractPlane_(
		...
		, _pcSource.getCM()._width
		, _pcSource.getCM()._height
...
void UBCDARTTracker::optimizePose(const int modelNum) {
	_optimizer->optimizePose(
		*_mirroredModels[modelNum],
