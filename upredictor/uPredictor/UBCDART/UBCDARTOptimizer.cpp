...
UBCDARTOptimizer::UBCDARTOptimizer(CameraModel &cm) {
	float2 focalLength;
	focalLength.x = cm._alpha;
	focalLength.y = cm._beta;
    init(cm._width,cm._height,focalLength,cm._width,cm._height);
}

void UBCDARTOptimizer::init(
	const int depthWidth, const int depthHeight, const float2 focalLength
...
void UBCDARTOptimizer::generateObsSdf(MirroredModel & model,
                               const Observation & observation,
                               const OptimizationOptions & opts) {
    generateObsSdfSplatZeroAndDistanceTransform(model,observation,opts);
}

void UBCDARTOptimizer::generateObsSdfSplatZeroAndDistanceTransform(MirroredModel & model,
                                                            const Observation & observation,
                                                            const OptimizationOptions & opts) {
...
void UBCDARTOptimizer::generateObsSdfDirectTruncated(MirroredModel & model,
                                              const Observation & observation,
                                              const OptimizationOptions & opts) {
...
void UBCDARTOptimizer::computeObsToModContribution(Eigen::VectorXf & eJ, Eigen::MatrixXf & JTJ, float & error,
...
void UBCDARTOptimizer::computeModToObsContribution(Eigen::VectorXf & eJ, Eigen::MatrixXf & JTJ, float & error,
                                            const MirroredModel & model, const Pose & pose,
...
void UBCDARTOptimizer::computeSelfIntersectionContribution(Eigen::VectorXf & eJ, Eigen::MatrixXf & JTJ, float & error,
                                                    const MirroredModel & model, const Pose & pose,
                                                    const OptimizationOptions & opts,
...
void UBCDARTOptimizer::optimizePose(
	MirroredModel & model,
	Pose & pose,
...
void UBCDARTOptimizer::optimizePoses(std::vector<MirroredModel *> & models,
